...
export const useWorkerCategoryCreation = ({navigation, route}: any) => {
  ...
  const handleSubmission = async () => {
    if (validate()) {
      ...
      await workerCategoryService.editWorkerCategory(
        parseInt(workerCategoryId),
        workerCategory,
      );
      if (redirect) {
  ...
  const hasUnsavedChanges =
    workerCategoryName.trim() !== workerCategoryList?.workerCategoryName ||
    notes.trim() !== workerCategoryList?.note;

  //  ||
  // workTypeList.length !== 0                 //noted
